...
energy of a molecular cluster to arbitrary order *n* using DFTB+ for the
...
* **Arbitrary MBE order** - choose *n*=1 (monomers) up to the number of
  fragments.
* **Massively parallel** - each subsystem is an embarrassingly parallel DFTB+ job;
  executed via *concurrent.futures*.
* **Robust restart** - existing DFTB+ outputs are reused; failed jobs are logged
  and skipped.
...
>>> python mbe.py cluster.xyz --order 3 --method "ri-mp2 def2-svp" \
        --charge 0 --multiplicity 1 --nprocs 8 --scratch /tmp/scratch 
...
def write_dftbplus_input(sym, xyz, sel, charge, path, parameters_path):
    """Create DFTB+ dftb_in.hsd file for selected fragment indices."""
    elements_set = set(sym)
    with path.open("w") as fh:
        fh.write("Geometry = GenFormat {\n")
        fh.write(f"{len(sel)} C\n")
        fh.write(f"{elements_set}\n")
        for i, index in enumerate(sel):
            # convert sym[index] to corresponding index of matching element in elements_set
            element_index = list(elements_set).index(sym[index]) + 1  # DFTB+ uses 1-based indexing for elements
            fh.write(f"{i+1} {element_index} {xyz[index,0]:.8f} {xyz[index,1]:.8f} {xyz[index,2]:.8f}\n")
        fh.write("}\n")
        fh.write("Driver {}\n")
        fh.write("Hamiltonian = DFTB {\n")
        fh.write(f"  Charge = {charge}\n")
        fh.write("  Scc = Yes\n")
        fh.write("  SlaterKosterFiles {\n")
        # load appropriate Slater-Koster files from all combinations of elements in elements_set 
        for elem1 in elements_set:
            for elem2 in elements_set:
                fh.write(f'    {elem1}-{elem2} = "{parameters_path}/{elem1}-{elem2}.skf"\n')
        fh.write("  }\n")
        ...
        fh.write("}\n")
        fh.write("Options {\n")
        fh.write("  WriteResultsTag = Yes\n")
        fh.write("}\n")
        fh.write("Analysis {\n")
        fh.write("  PrintForces = Yes\n")
        fh.write("}\n")
        fh.write("ParserOptions {\n")
        fh.write("  ParserVersion = 14\n")
        fh.write("}\n")
...
def main(argv: Optional[Sequence[str]] = None):
    ...
    if args.fragments:
        ...
        raise Fatal("Fragment definitions via --fragments JSON are required.")
    n_frag = len(frags)
